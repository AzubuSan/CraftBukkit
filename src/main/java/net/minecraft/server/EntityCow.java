...
public class EntityCow extends EntityAnimal {
    ...
    protected void a(boolean flag, int i) {
        ...
        int count = this.random.nextInt(3) + this.random.nextInt(1 + i);

        if (count > 0) {
            loot.add(new org.bukkit.inventory.ItemStack(Item.LEATHER.id, count));
        }

        count = this.random.nextInt(3) + 1 + this.random.nextInt(1 + i);

        if (count > 0) {
            loot.add(new org.bukkit.inventory.ItemStack(this.z() ? Item.COOKED_BEEF.id : Item.RAW_BEEF.id, count));
        }
